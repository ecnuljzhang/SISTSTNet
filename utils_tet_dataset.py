...
import os
from tet_data_augmentation import generate_styles
import random
...
def load_trainset_batchfnames(filepath, batch_size, usetrainnum=708, trainnum=100000):
    ...
    stylenum = len(paths)
    trainnum = int((trainnum / batch_size / 2) * batch_size * 2)
    # print(trainnum)
    fnames = ['%s.png' % int(i%usetrainnum+1) for i in range(trainnum)]
    pathid = [int(i%stylenum) for i in range(trainnum//2)]
...
# filepath 在这里是风格的路径
def load_trainset_fnames(filepath, batch_size, usetrainnum=708, trainnum=100000):
    batch_size = 2
    paths = os.listdir(filepath)
    if '.DS_Store' in paths:
        paths.remove('.DS_Store')
    stylenum = len(paths) # 风格数量
    trainnum = (trainnum // batch_size // 2) * batch_size * 2 # 设定的数量,使其成为batchsize*2的整数倍
    fnames = ['%s.png' % (i%usetrainnum+1) for i in range(trainnum)] # 文件名字从0开始到708
    pathid = [(i%stylenum) for i in range(trainnum//2)] # 风格的名字是数字从0到stylenum
    random.shuffle(pathid)
    random.shuffle(fnames)
    trainbatches = []
    for i in range(trainnum//batch_size//2):
        ...
        for j in range(batch_size):
            ...
            traindatas = [[os.path.join(filepath, paths[pathid[ii//2]], 'train', fnames[ii]),
                          os.path.join(filepath, paths[pathid[(ii+1)//2]], 'train', fnames[ii+1])]]  
            trainbatches.append(traindatas)
    return trainbatches # 一次随机选两张图

# 和trainset 一模一样
def load_validset_fnames(filepath,
                         batch_size, usetrainnum=708, trainnum=100000):
    batch_size = 2
    paths = os.listdir(filepath)
    if '.DS_Store' in paths:
        paths.remove('.DS_Store')
    ...
    for i in range(trainnum//batch_size//2):
        ...
        for j in range(batch_size):
            ii = i * batch_size + j * 2
            traindatas = [[os.path.join(filepath, paths[pathid[ii//2]], 'valid', fnames[ii]),
                          os.path.join(filepath, paths[pathid[(ii+1)//2]], 'valid', fnames[ii+1])]]  
            trainbatches.append(traindatas)
    return trainbatches

# prepare batched filenames of all training data
def load_valid_batchfnames(filepath, batch_size, usetrainnum=708, trainnum=100000):
    paths = os.listdir(filepath)
    stylenum = len(paths)
    trainnum = int((trainnum / batch_size / 2) * batch_size * 2)
    # print(trainnum)
...
def prepare_batch(batchfnames, level=3, jitter=0.0, centercropratio=0.5, augementratio=0.0, gpu=True, wd=256, ht=256):
    # (self._file_names[idx], 2, 0/1, 0.5, 0.25, 1)
    transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.5, 0.5, 0.5],std=[0.5,0.5,0.5]),
    ])
    ...
    for fname1, fname2 in batchfnames: # 默认图像是两张拼在一起，左边是输入，右边是gt
    ...
    outs2 = []  # y'
    for i in range(layernum):
        data1 = (torch.cat(img_list[i], dim=0))
        data2 = (torch.cat(img_list2[i], dim=0))
